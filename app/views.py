...
def register():
    ...
    if form.validate_on_submit():
        ...
        user = User(firstname=form.firstname.data, lastname=form.lastname.data, email=form.email.data, password=generate_password_hash(form.password.data))
        db.session.add(user)
        db.session.commit()
        flash('Thanks for registering. Your account will need activation.', category='info')
        return redirect(url_for('login'))
...
def calculateHours(uInitial, sem, userstring):
    tuthours = 0
    lecthours = 0
    labhours = 0
    markhours = 0
    ...
    entries = EntryClone.query.filter_by(userInitial=uInitial, SemesterId=sem)
    entries = entries.filter((EntryClone.CourseType == 'TUTA') | (EntryClone.CourseType == 'TUTB') | (EntryClone.CourseType == 'TUT')).all()
    for e in entries:
    ...
    users.append(tuthours)
    markings = Marking.query.filter_by(userInitial=uInitial, SemesterId=sem).all()
    for m in markings:
        markhours = markhours + m.SubmissionNumber * m.GradingRate / 60.0

    users.append(markhours)
    lectures = EntryClone.query.filter_by(userInitial=uInitial, SemesterId=sem, CourseType='LEC').all()
    for l in lectures:
    ...
    users.append(lecthours)
    labs = EntryClone.query.filter_by(userInitial=uInitial, SemesterId=sem, CourseType='LAB').all()
    for b in labs:
    ...
    for (index, time,) in enumerate(times):
        ...
        for day in days:
            ...
            if tutorial:
                ...
                values = values.filter((EntryClone.CourseType == 'LAB') | (EntryClone.CourseType == 'TUTA') | (EntryClone.CourseType == 'TUTB') | (EntryClone.CourseType == 'TUT')).all()
...
def addMarker():
    ...
    form = MarkingForm()
    if form.validate_on_submit():
        entry = Marking(CourseCode=form.CourseCode.data, userInitial=form.UserInitial.data, SemesterId=form.SemesterId.data, AssignmentCode=form.AssignmentCode.data, AssignmentDue=form.AssignmentDue.data, MarkingDue=form.AssignmentDue.data, SubmissionNumber=form.SubmissionNumber.data, GradingRate=form.GradingRate.data)
...
def deleteT(id):
    user = User.query.get_or_404(g.user.id)
    entry = EntryClone.query.get_or_404(id)
    slotinfo = entry.CourseCode + ' ' + entry.EntryDay + '@' + entry.EntryTime
    if not entry:
        abort(404)
    ...
    db.session.add(entry)
    db.session.commit()
    flash('Your Entry was successfully removed: ' + slotinfo, category='success')
...
def addT(id):
    ...
    db.session.add(entry)
    db.session.commit()
    flash('Your Entry was successfully added: ' + slotinfo, category='success')
    myurl = app.config['SERVER_BASE'] + '/timetable/' + entry.SemesterId + '?btn=edit'
    return redirect(myurl)
...
def timeDifference(start_time, end_time):
    ...
    return int(time_difference / 3600.0)
    ...
    if form.validate_on_submit():
        ...
        for key in readerDict:
            ...
            for i in range(timeDiff):
                entry = EntryClone(EntryDay=entry_day, EntryTime=times[(indx + i)], RoomId=entry_room, SemesterId=entry_sem, CourseCode=entry_course, CourseType=entry_type, userInitial=entry_initial)
                db.session.add(entry)
                db.session.commit()
...
def manageusers():
    ...
    for user in users:
        ...
        userdict['' + user.userInitial] = calculateHours(user.userInitial, sem, user)

    users = User.query.order_by(User.lastname).all()
    return render_template('manageusers.html', title=title, users=users, userdict=userdict)
...
def edituser(id):
    ...
    return render_template('edituser.html', user=user, title=title, form=form)
...
def get_recipes():
    recipes = [{'id': 1,
      ...
      'instructions': ['1. Boil Chunks', '2. Drain', '3. Mix in Curry']}, {'id': 2,
